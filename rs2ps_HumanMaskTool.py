...
rs2ps_HumanMaskTool.py
  ...
  - Performance settings (score, morphology, resize) and shadow parameters are fixed constants; only --include_shadow and --dilate are configurable.
"""
...
def process_image(model, device, in_path: Path, out_dir: Path,
                  ...
                  close: int = CLOSE_KERNEL, dilate_ratio: float = 0.01, mode: str = "mask",
...
def main():
    ...
    ap.add_argument("--dilate", type=float, default=0.0,
                    help="Dilation kernel size as a percentage of the image size (0 disables)")
    ...
    dilate_ratio = max(float(args.dilate), 0.0) / 100.0
    ...
    for idx, p in enumerate(images, 1):
        ...
        process_image(model, device, p, out_dir,
                      ...
                      close=CLOSE_KERNEL, dilate_ratio=dilate_ratio,
